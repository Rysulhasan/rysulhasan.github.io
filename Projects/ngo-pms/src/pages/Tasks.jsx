...
import { tasksAPI, subtasksAPI, projectsAPI, usersAPI, reportsAPI, taskHeadsAPI } from '../services/api';
...
const Tasks = () => {
  ...
  const loadData = async () => {
    try {
      const [tasksData, projectsData, usersData, taskHeadsData] = await Promise.all([
  ...
  const getTaskHeadsForProject = (projectId) => {
    return taskHeads.filter(th => th.projectId === projectId);
  };
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <select
            ...
            onChange={(e) => {
              ...
              setTaskHeadFilter('all'); // Reset task head filter when project changes
            }}
                  ...
                  <div className="flex flex-wrap items-center gap-4 mt-3 text-sm text-text-secondary">
                    ...
                    {task.taskHeadId && (
                      <span 
                        ...
                        style={{ 
                          backgroundColor: `${getTaskHead(task.taskHeadId)?.color}20`, 
                          color: getTaskHead(task.taskHeadId)?.color 
                        }}
                      >
                        <Tag className="w-3 h-3" />
                        {getTaskHead(task.taskHeadId)?.name}
                      </span>
                    )}
            ...
            <form onSubmit={handleTaskSubmit} className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setTaskForm({ ...taskForm, projectId: e.target.value, taskHeadId: '' })}
              ...
              <div>
                <label htmlFor="taskHeadId" className="label">Task Head / Tag (optional)</label>
                <select
                  id="taskHeadId"
                  value={taskForm.taskHeadId}
                  onChange={(e) => setTaskForm({ ...taskForm, taskHeadId: e.target.value })}
                  className="input-field"
                  disabled={!taskForm.projectId}
                >
                  <option value="">No task head selected</option>
                  {getTaskHeadsForProject(taskForm.projectId).map(head => (
                    <option key={head.id} value={head.id}>{head.name}</option>
                  ))}
                </select>
                {!taskForm.projectId && (
                  <p className="text-xs text-text-secondary mt-1">Select a project first to see available task heads</p>
                )}
              </div>
