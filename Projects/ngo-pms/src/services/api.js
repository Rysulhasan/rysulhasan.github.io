...
import { demoUsers, demoProjects, demoTasks, demoSubTasks, demoReports, demoTaskHeads } from '../data/demoData';
...
const initializeData = () => {
  if (!localStorage.getItem(STORAGE_KEYS.INITIALIZED)) {
    ...
    localStorage.setItem(STORAGE_KEYS.TASK_HEADS, JSON.stringify(demoTaskHeads));
...
export const taskHeadsAPI = {
  async getAll(filters = {}) {
    await delay(200);
    let taskHeads = getData(STORAGE_KEYS.TASK_HEADS);
    
    if (filters.projectId) {
      taskHeads = taskHeads.filter(th => th.projectId === filters.projectId);
    }
    
    return taskHeads.sort((a, b) => a.order - b.order);
  },
  
  async getById(id) {
    await delay(100);
    const taskHeads = getData(STORAGE_KEYS.TASK_HEADS);
    const taskHead = taskHeads.find(th => th.id === id);
    if (!taskHead) {
  ...
  async create(taskHeadData) {
    await delay(300);
    const taskHeads = getData(STORAGE_KEYS.TASK_HEADS);
    
    // Get max order for this project
    const projectTaskHeads = taskHeads.filter(th => th.projectId === taskHeadData.projectId);
    const maxOrder = projectTaskHeads.length > 0 ? Math.max(...projectTaskHeads.map(th => th.order)) : 0;
    
    const newTaskHead = {
      id: generateId('head'),
      projectId: taskHeadData.projectId,
      name: taskHeadData.name,
      color: taskHeadData.color || '#3B82F6',
      description: taskHeadData.description || '',
      order: maxOrder + 1,
      createdAt: new Date().toISOString()
    };
    
    taskHeads.push(newTaskHead);
    setData(STORAGE_KEYS.TASK_HEADS, taskHeads);
    return newTaskHead;
  },
  
  async update(id, taskHeadData) {
    await delay(300);
    const taskHeads = getData(STORAGE_KEYS.TASK_HEADS);
    const taskHeadIndex = taskHeads.findIndex(th => th.id === id);
    
    if (taskHeadIndex === -1) {
      throw new Error('Task head not found');
    }
    
    taskHeads[taskHeadIndex] = {
      ...taskHeads[taskHeadIndex],
      ...taskHeadData
    };
    
    setData(STORAGE_KEYS.TASK_HEADS, taskHeads);
    return taskHeads[taskHeadIndex];
  },
  
  async delete(id) {
    await delay(300);
    const taskHeads = getData(STORAGE_KEYS.TASK_HEADS);
    const filteredTaskHeads = taskHeads.filter(th => th.id !== id);
    
    if (filteredTaskHeads.length === taskHeads.length) {
      throw new Error('Task head not found');
    }
    
    // Update tasks that had this taskHeadId to null
    const tasks = getData(STORAGE_KEYS.TASKS);
    const updatedTasks = tasks.map(t => t.taskHeadId === id ? { ...t, taskHeadId: null } : t);
    setData(STORAGE_KEYS.TASKS, updatedTasks);
    
    setData(STORAGE_KEYS.TASK_HEADS, filteredTaskHeads);
    return { success: true };
  },
  ...
  async reorder(projectId, orderedIds) {
    ...
    orderedIds.forEach((id, index) => {
      const taskHeadIndex = taskHeads.findIndex(th => th.id === id);
      if (taskHeadIndex !== -1) {
        taskHeads[taskHeadIndex].order = index + 1;
      }
    });
...
export const tasksAPI = {
  async getAll(filters = {}) {
    ...
    if (filters.taskHeadId) {
      tasks = tasks.filter(t => t.taskHeadId === filters.taskHeadId);
    }
  ...
  async create(taskData) {
    ...
    const newTask = {
      ...
      taskHeadId: taskData.taskHeadId || null,
